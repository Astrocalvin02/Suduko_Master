...
void playAgainPrompt(int size, int nobs, const std::string& solver_method){
    ...
    if (user_response == "y"){
        playGame(size, nobs, solver_method);
    } else {
        return;
    }
}
...
void playGame(int size, int nobs, const std::string& solver_method){
    ...
    do {
        ...
        if (user_entry == "Solve" || user_entry == "solve"){
            if (solver_method == "rp") {
                cout << "Solving with Random Projections..." << endl;
                // Board solved_board = DR(board);
                // solverSucces = solved_board.checkPuzzle();
                // if (solverSucces) board = solved_board;
                cout << "Random Projections solver is unavailable in this build." << endl;
            } else {
    ...
    if (solverSucces){
        ...
        playAgainPrompt(size, nobs, solver_method);
